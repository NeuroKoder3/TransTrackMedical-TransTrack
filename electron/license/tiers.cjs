...
function getCurrentBuildVersion() {
  ...
  // Check for build marker file. Electron may be unavailable in Node-only
  // test harnesses — never let a missing binary crash license checks.
  try {
    const fs = require('fs');
    ...
    // Fallback to evaluation for safety (fail closed on entitlements)
  }
...
const PAYMENT_CONFIG = {
  businessEmail: 'billing@transtrack.health',
  contactEmail: 'support@transtrack.health',
...
For purchase orders, wire transfers, or other payment methods:
  Email: billing@transtrack.health
